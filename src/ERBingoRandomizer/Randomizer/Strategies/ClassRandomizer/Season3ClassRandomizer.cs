...
public class Season3ClassRandomizer : IBingoClassStrategy {
    ...
    private void logCharaInitEntry(Params.CharaInitParam chr, int i) {
        ...
        if (chr.subWepLeft != -1) {
            Logger.LogItem(
                $"Left 2: {_resources.WeaponFmg[0][chr.subWepLeft]}{getRequiredLevelsWeapon(chr, chr.subWepLeft)}");
        }

        if (chr.subWepRight != -1) {
            Logger.LogItem(
                $"Right 2: {_resources.WeaponFmg[0][chr.subWepRight]}{getRequiredLevelsWeapon(chr, chr.subWepRight)}");
        }

        if (chr.subWepLeft3 != -1) {
            Logger.LogItem(
                $"Left 3: {_resources.WeaponFmg[0][chr.subWepLeft3]}{getRequiredLevelsWeapon(chr, chr.subWepLeft3)}");
        }

        if (chr.subWepRight3 != -1) {
            Logger.LogItem(
                $"Right 3: {_resources.WeaponFmg[0][chr.subWepRight3]}{getRequiredLevelsWeapon(chr, chr.subWepRight3)}");
        }
        ...
        if (chr.equipSpell01 != -1 || chr.equipSpell02 != -1) {
            ...
            if (chr.equipSpell01 != -1) {
                Logger.LogItem(
                    $"{_resources.GoodsFmg[0][chr.equipSpell01]}{getRequiredLevelsSpell(chr, chr.equipSpell01)}");
            }

            if (chr.equipSpell02 != -1) {
                Logger.LogItem(
                    $"{_resources.GoodsFmg[0][chr.equipSpell02]}{getRequiredLevelsSpell(chr, chr.equipSpell02)}");
            }
        }
    ...
    private void addDescriptionString(Params.CharaInitParam chr, int id) {
        for (var i = 0; i < Const.ERLanguageCount; i++) {
            ...
            if (chr.subWepRight != -1) {
                str.Add(
                    $"{_resources.WeaponFmg[i][chr.subWepRight]}{getRequiredLevelsWeapon(chr, chr.subWepRight)}");
            }

            if (chr.subWepLeft3 != -1) {
                str.Add(
                    $"{_resources.WeaponFmg[i][chr.subWepLeft3]}{getRequiredLevelsWeapon(chr, chr.subWepLeft3)}");
            }

            if (chr.subWepRight3 != -1) {
                str.Add(
                    $"{_resources.WeaponFmg[i][chr.subWepRight3]}{getRequiredLevelsWeapon(chr, chr.subWepRight3)}");
            }
